...
carto.Parser = function Parser(env) {
    ...
    function makeError(err) {
        ...
        if (!_.isNil(err.filename)) {
            if (that.env.inputs && that.env.inputs[err.filename]) {
            ...
            if (!_.isNil(err.index)) {
                ...
                return new Error(_.template('<%=filename%> <%=message%>')(err));
            }
        }
        else {
            return new Error(_.template('<%=message%>')(err));
        }
    }
                ...
                return function(env) {
                    env.error = function(e) {
                        ...
                        if (env.errors.message) {
                            var msg = makeError(e).message;
                            // only add error message if different from already present message
                            if (env.errors.message != msg) {
                                env.errors.message += '\n' + msg;
                            }
                        } else {
                            env.errors.message = makeError(e).message;
                        }
                        if (!env.errors.filename) {
                            env.errors.filename = makeError(e).filename;
                        }
                    };
